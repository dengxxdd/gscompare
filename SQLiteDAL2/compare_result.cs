...
namespace DXD.SQLiteDAL
{
	...
	public partial class compare_result:Icompare_result
	{
		...
		public int Add(DXD.Model.compare_result model)
		{
			...
			strSql.Append("query_id,query_type,report_str,feedback_str,compare_str,add_user,add_time)");
			strSql.Append(" values (");
			strSql.Append("@query_id,@query_type,@report_str,@feedback_str,@compare_str,@add_user,@add_time)");
			strSql.Append(";select LAST_INSERT_ROWID()");
			SQLiteParameter[] parameters = {
					new SQLiteParameter("@query_id", DbType.Int32,8),
					new SQLiteParameter("@query_type", DbType.Int32,4),
			...
			parameters[1].Value = model.query_type;
		...
		public DXD.Model.compare_result GetModel(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,query_id,query_type,report_str,feedback_str,compare_str,add_user,add_time from compare_result ");
			strSql.Append(" where id=@id");
			SQLiteParameter[] parameters = {
					new SQLiteParameter("@id", DbType.Int32,4)
			};
		...
		public DXD.Model.compare_result DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
				...
				if(row["query_type"]!=null && row["query_type"].ToString()!="")
				{
					model.query_type=int.Parse(row["query_type"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,query_id,query_type,report_str,feedback_str,compare_str,add_user,add_time ");
