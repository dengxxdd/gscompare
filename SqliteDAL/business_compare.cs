...
namespace DXD.SQLiteDAL
{
	...
	public partial class business_compare:Ibusiness_compare
	{
		...
		public DXD.Model.business_compare GetModel(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,query_id,feedback_id,report_id,compare_type,less_value,add_user,add_time from business_compare ");
			strSql.Append(" where id=@id");
			SQLiteParameter[] parameters = {
					new SQLiteParameter("@id", DbType.Int32,4)
			};
		...
		public DXD.Model.business_compare DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
				...
				if(row["compare_type"]!=null)
				{
					model.compare_type=row["compare_type"].ToString();
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,query_id,report_id,rep_full_name,rep_bus_name,rep_bus_type,rep_subscribe," +
                "rep_proportion,rep_office, rep_status, feedback_id, fee_full_name, fee_bus_name, fee_bus_type, " +
                "fee_subscribe,fee_proportion,fee_office, fee_status, compare_type, less_value ");
			strSql.Append(" FROM vGetBusinessCompare ");
			if(strWhere.Trim()!="")
			{
